...
public class checkSAPHanaNetStatistics extends checkTemplate{

	String SQL_for_list = "";

	public checkSAPHanaNetStatistics(globalData gData, batchJob job) {
		super(gData, job);
	...
	public void executeCheck() {
		...
		gData.logger.info("checkSAPHanaNetStatistics for " + systems.size());
	...
	private void remoteCheck(remoteSystem system) {
		...
		if(db_version.contains("ORACLE")) {
			...
			checkHanaNetStatistics(system);

		} else {
			system.params.put("result_text", "Check canceled. Type DB " + db_version + " is unknown");
		}
	}
	private void checkHanaNetStatistics(remoteSystem s) {
		...
		List<Map<String , String>> records_prep =  
				gData.sqlReq.getSelect(gData.commonParams.get("connectionString") , gData.commonParams.get("user") , gData.commonParams.get("password"), "select param_value from monitor_ext_data where param_name='SqlHanaNetStat' and object_guid='" + s.params.get("guid") + "'");
